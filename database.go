...
// HasBanActions reports whether any ban action exists within the report window
// ending at `now + offsetDays` and spanning `numDays` days back.
func (d *Database) HasBanActions(offsetDays, numDays int) (exists bool, err error) {
	timestamp := time.Now().AddDate(0, 0, offsetDays)

	var count int64
	res := d.db.Model(&BanActionLog{}).
		Where("created_at >= ? AND created_at < ?", timestamp.AddDate(0, 0, -numDays), timestamp).
		Count(&count)
...
func (d *Database) generateReport(offsetDays, numDaysForReport1, numDaysForReport2 int) (result Report, err error) {
	...
	if res := d.db.Model(&BanActionLog{}).Where("created_at >= ? AND created_at < ?", timestamp.AddDate(0, 0, -numDaysForReport1), timestamp).Find(&lastDaysForReport1); res.Error == nil {
		// total count
	...
	if res := d.db.Model(&BanActionLog{}).Where("created_at >= ? AND created_at < ?", timestamp.AddDate(0, 0, -numDaysForReport2), timestamp).Find(&lastDaysForReport2); res.Error == nil {
...
<i>report generated by <a href="%[11]s">balog</a></i>`,
			report.LastDaysReport1.FromTo, numDaysForReport1, report.LastDaysReport1.TotalCount, strings.Join(protocolsForReport1, "\n"), strings.Join(countriesForReport1, "\n"),
			report.LastDaysReport2.FromTo, numDaysForReport2, report.LastDaysReport2.TotalCount, strings.Join(protocolsForReport2, "\n"), strings.Join(countriesForReport2, "\n"),
			projectURL,
		)
